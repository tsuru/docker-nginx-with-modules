...
test: image
	...
	@docker create -p 8888:80 --name test-tsuru-nginx-$(flavor)-$(nginx_version) tsuru/nginx-$(flavor):$(nginx_version) nginx -c /etc/nginx/nginx-$(flavor).conf
	...
	@docker start test-tsuru-nginx-$(flavor)-$(nginx_version)
